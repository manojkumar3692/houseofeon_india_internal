...
export function storeTerms() {
  ...
  return {
    ...
    promotions: { status: 'known', offers: coupon?.active ? [{
      code: coupon.code,
      description: `${coupon.value}% off perfume cart subtotal; discount rounded to nearest whole INR (Math.round), capped at INR ${coupon.maxDiscount ?? 'subtotal'}. Minimum INR ${coupon.minSubtotal ?? 0}. Shopper must explicitly enter EON20; it is never automatically applied. Excludes 2+ bottle bundle pricing, trial sets and negotiated offers. One code per order; no stacking with other codes or trial credit. No expiry is configured.`,
